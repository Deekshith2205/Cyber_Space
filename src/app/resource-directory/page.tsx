...
const Card = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <motion.div
    ...
    className={`glass border border-white/10 p-5 group transition-all duration-300 hover:border-cyber-blue/50 hover:shadow-[0_0_20px_rgba(0,229,255,0.15)] ${className}`}
  >
    {children}
  </motion.div>
);
...
const SectionTitle = ({ title }: { title: string }) => (
  <div className="flex items-center gap-3 mb-6">
    <div className="h-1 w-8 bg-cyber-blue rounded-full" />
    <h2 className="text-xl font-bold text-white tracking-tight uppercase tracking-[0.1em]">{title}</h2>
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-background grid-background p-6 lg:p-10 space-y-12 pb-20">
      ...
      <section className="space-y-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="relative group max-w-3xl">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-cyber-blue transition-colors group-focus-within:drop-shadow-[0_0_8px_rgba(0,229,255,0.5)]" size={20} />
            <input
              type="text"
              placeholder="Search cybersecurity tools, CVEs, databases..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-cyber-blue focus:ring-1 focus:ring-cyber-blue/30 transition-all text-sm text-white focus:shadow-[0_0_15px_rgba(0,229,255,0.1)]"
            />
          </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                  activeCategory === filter 
                    ? "bg-cyber-blue/20 border-cyber-blue text-cyber-blue shadow-[0_0_10px_rgba(0,229,255,0.2)]" 
        ...
        <AnimatePresence mode="popLayout">
          {filteredResources.length > 0 || filteredKnowledge.length > 0 ? (
                ...
                >
                  <Card className="h-full border-neon-purple/30">
                    <div className="flex flex-col h-full">
                      ...
                      <p className="text-zinc-400 text-sm mb-6 flex-grow">{item.shortDesc}</p>
                      <div className="flex items-center text-[10px] text-zinc-500 font-bold uppercase tracking-widest group-hover:text-neon-purple transition-colors">
                  ...
                  <div className="flex flex-col h-full">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest">{resource.category}</span>
                        {resource.verified && (
                    ...
                    <p className="text-zinc-400 text-sm mb-6 flex-grow">{resource.description}</p>
                    {resource.region && (
                      <span className="text-[10px] text-zinc-600 font-bold mb-4 uppercase tracking-tighter">Region: {resource.region}</span>
                    )}
                    <button 
                      onClick={() => handleOpenResource(resource.link)}
                      className="flex items-center justify-center gap-2 w-full py-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-white hover:bg-cyber-blue hover:text-black transition-all group"
                    >
                      Open Resource
            ...
            <motion.div 
              ...
              className="flex flex-col items-center justify-center py-20 text-center space-y-4 glass border-white/5"
            >
              <div className="p-6 bg-white/5 rounded-full text-zinc-600">
                <SearchX size={48} />
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">No cybersecurity resources found</h3>
                <p className="text-zinc-500 max-w-xs mx-auto">Try adjusting your search terms or filter to find what you're looking for.</p>
              </div>
              <button 
                onClick={() => { setSearchQuery(""); setActiveCategory("All"); }}
                className="px-6 py-2 bg-cyber-blue/10 border border-cyber-blue/20 rounded-xl text-xs font-bold text-cyber-blue hover:bg-cyber-blue hover:text-black transition-all"
              >
                Clear All Filters
              </button>
            </motion.div>
      ...
      <section className="border-t border-white/5 pt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <motion.div 
            ...
            className="glass p-6 min-h-[400px] flex flex-col"
          >
            <h3 className="text-sm font-bold text-zinc-500 mb-6 uppercase tracking-widest text-center">Threat Category Distribution</h3>
          ...
          <motion.div 
            ...
            className="glass p-6 min-h-[400px] flex flex-col"
          >
            <h3 className="text-sm font-bold text-zinc-500 mb-6 uppercase tracking-widest text-center">Security Tool Usage</h3>
            <div className="flex-grow">
              <ResponsiveContainer width="100%" height={300}>
      ...
      <section className="border-t border-white/5 pt-10">
        <h3 className="text-sm font-bold text-zinc-500 mb-6 uppercase tracking-widest text-center">Top Recommended Tools</h3>
        <div className="flex flex-wrap justify-center gap-4">
          {recommendedTools.map((tool, idx) => (
            <motion.div
              ...
              className="px-6 py-3 glass border border-white/10 rounded-2xl flex items-center gap-2 cursor-pointer hover:border-cyber-blue transition-all"
            >
        ...
        {selectedKnowledge && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div 
              ...
              className="glass max-w-2xl w-full p-8 border border-white/10 relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="p-4 bg-neon-purple/20 rounded-2xl text-neon-purple shadow-[0_0_20px_rgba(122,92,255,0.2)]">
                  <selectedKnowledge.icon size={32} />
                </div>
                <div>
                  <h2 className="text-3xl font-black text-white">{selectedKnowledge.title}</h2>
                  <span className="text-xs font-bold text-neon-purple uppercase tracking-widest">Threat Analysis</span>
                </div>
              </div>

              <div className="space-y-6">
                <div>
                  <h4 className="text-sm font-bold text-zinc-400 mb-2 uppercase tracking-widest flex items-center gap-2">
                    <Info size={14} className="text-neon-purple" /> Description
                  </h4>
                  <p className="text-zinc-300 leading-relaxed">{selectedKnowledge.fullDesc}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-4 bg-success-green/5 border border-success-green/10 rounded-2xl">
                    <h4 className="text-xs font-black text-success-green mb-3 uppercase tracking-widest">Prevention Tips</h4>
                    <p className="text-zinc-400 text-sm leading-relaxed">{selectedKnowledge.prevention}</p>
                  </div>
                  <div className="p-4 bg-cyber-blue/5 border border-cyber-blue/10 rounded-2xl">
                    <h4 className="text-xs font-black text-cyber-blue mb-3 uppercase tracking-widest">Detection Tools</h4>
                    <p className="text-zinc-400 text-sm leading-relaxed">{selectedKnowledge.tools}</p>
                  </div>
                </div>

                <button 
                  onClick={() => setSelectedKnowledge(null)}
                  className="w-full py-4 mt-4 bg-white/5 border border-white/10 rounded-2xl font-bold text-white hover:bg-white/10 transition-all"
                >
                  Dismiss
                </button>
              </div>
