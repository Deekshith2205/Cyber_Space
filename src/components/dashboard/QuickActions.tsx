            ...
            {actions.map((action) => (
                <Link key={action.id} href={action.href} className="block group">
                    <motion.div
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="glass p-5 rounded-3xl cursor-pointer relative overflow-hidden transition-all duration-400 shadow-depth border-none h-full"
                    >
                        <div className={cn(
